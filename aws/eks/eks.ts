...
export const cluster = new eks.Cluster(`${eksClusterName}-cluster`, {
  ...
  enabledClusterLogTypes: [               // Enable cluster logging for key components
    "api",
    "audit",
    "authenticator",
    "controllerManager",
  ...
  storageClasses: {                       // Define default storage classes for the cluster
    gp2: {
...
eks.createManagedNodeGroup(`${eksClusterName}-node-group`, {
  ...
  scalingConfig: {                        // Configure the scaling parameters for the node group
